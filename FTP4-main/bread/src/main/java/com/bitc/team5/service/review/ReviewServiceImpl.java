...
import com.bitc.team5.mapper.review.ReviewMapper;
...
public class ReviewServiceImpl implements ReviewService {
	...
	@Override
	public void storeinsert(ReviewDto userreview, String userId) {
			
		userreview.setUserId(userId);
		
		reviewMapper.storeinsert(userreview);
	...
	public void writeReview(ReviewDto review, MultipartHttpServletRequest multiFiles, String reviewCode, String userId) throws Exception {
		...
		List<AdminProductFileDto> alist = fileUtils.parseFileInfo(review.getIdx(), multiFiles);
		
		
		for (AdminProductFileDto item : alist) {
				item.setReviewCode(reviewCode);
	...
	public ReviewDto selectReview(int idx ) throws Exception {
		...
		ReviewDto board = reviewMapper.selectReview(  idx );

		// 첨부파일
		List<AdminProductFileDto> fileList = reviewMapper.showReviewList( idx);
	...
	public List<BoardDto> selectMyReview(String userId) throws Exception {
		return reviewMapper.selectMyReview(userId);
	}
//	@Autowired
//	private ProductFileUtils fileUtils;
//	
